...
_COMP_RE = re.compile(r"^(0[1-9]|1[0-2])/[0-9]{4}$")
_ALLOWED_STATUS_APROVACAO = {"rascunho", "em_aprovacao", "aprovado", "rejeitado"}
...
def _normalize_competencia(value) -> str:
    ...
    if len(parts) == 2:
        ...
        if len(a) == 4 and len(b) <= 2:
            ...
            txt = f"{int(a):02d}/{b}"
    if not _COMP_RE.match(txt):
        raise ValueError("Competência deve estar no formato MM/AAAA.")
    return txt


def _normalize_status(value, default="rascunho") -> str:
    ...
    if txt not in _ALLOWED_STATUS_APROVACAO:
...
class ProdutividadeParametroService(BaseProdService):
    ...
    def _validate(self, d):
        d["disciplina"] = str(d.get("disciplina", "")).strip()
        d["horas_por_a1"] = to_float(d.get("horas_por_a1", 0))
        d["custo_hora_equipe"] = to_float(d.get("custo_hora_equipe", 0))
        d["ativo"] = 1 if str(d.get("ativo", "Sim")).lower() in {"1", "sim", "true", "yes"} else 0
        if not d["disciplina"]:
            raise ValueError("Informe a disciplina.")
        if d["horas_por_a1"] < 0 or d["custo_hora_equipe"] < 0:
            raise ValueError("Horas/A1 e custo-hora não podem ser negativos.")
        return d

    def create(self, d):
        d = self._validate(d)
        new_id = self.repo.create(d)
        self.log(new_id, "create", f"Parâmetro criado: {d['disciplina']}")

    def update(self, obj_id, d):
        d = self._validate(d)
        self.repo.update(obj_id, d)
        self.log(obj_id, "update", f"Parâmetro atualizado: {d['disciplina']}")

    def delete(self, obj_id):
        self.log(obj_id, "delete", "Parâmetro excluído")
...
class ProdutividadeMetaService(BaseProdService):
    def __init__(self, repo, usuario_repo, contrato_repo, etapa_repo, grupo_repo, parametro_repo, historico_repo=None, app_context=None):
        ...
        self.parametro_repo = parametro_repo
    ...
    def _validate(self, d):
        d["usuario_id"] = int(d.get("usuario_id", 0))
        ...
        d["competencia"] = _normalize_competencia(d.get("competencia", ""))
        d["disciplina"] = str(d.get("disciplina", "")).strip()
        d["meta_mensal_a1"] = to_float(d.get("meta_mensal_a1", 0))
        d["horas_por_a1"] = to_float(d.get("horas_por_a1", 0))
        d["receita_prevista"] = to_float(d.get("receita_prevista", 0))
        d["status"] = str(d.get("status", "planejado")).strip() or "planejado"
        if d["horas_por_a1"] == 0:
            prm = self.parametro_repo.get_by_disciplina(d["disciplina"])
            if prm:
                d["horas_por_a1"] = float(prm["horas_por_a1"] or 0)
        ...
        if d["meta_mensal_a1"] < 0 or d["horas_por_a1"] < 0 or d["receita_prevista"] < 0:
            raise ValueError("Meta, horas e receita prevista não podem ser negativas.")
        return d

    def create(self, d):
        d = self._validate(d)
        new_id = self.repo.create(d)
        self.log(new_id, "create", "Meta criada")

    def update(self, obj_id, d):
        d = self._validate(d)
        self.repo.update(obj_id, d)
        self.log(obj_id, "update", "Meta atualizada")

    def delete(self, obj_id):
        self.log(obj_id, "delete", "Meta excluída")
        self.repo.delete(obj_id)
...
class ProdutividadeRealizadoService(BaseProdService):
    def __init__(self, repo, usuario_repo, contrato_repo, etapa_repo, grupo_repo, entregavel_repo, parametro_repo, historico_repo=None, app_context=None):
        ...
        self.etapa_repo = etapa_repo
        self.grupo_repo = grupo_repo
        self.entregavel_repo = entregavel_repo
        self.parametro_repo = parametro_repo
    ...
    def _validate(self, d):
        ...
        d["competencia"] = _normalize_competencia(d.get("competencia", ""))
        d["disciplina"] = str(d.get("disciplina", "")).strip()
        d["produzido_a1"] = to_float(d.get("produzido_a1", 0))
        d["horas_por_a1"] = to_float(d.get("horas_por_a1", 0))
        d["receita_faturada"] = to_float(d.get("receita_faturada", 0))
        d["status_aprovacao"] = _normalize_status(d.get("status_aprovacao", "rascunho"))
        if d["horas_por_a1"] == 0:
            prm = self.parametro_repo.get_by_disciplina(d["disciplina"])
            if prm:
                d["horas_por_a1"] = float(prm["horas_por_a1"] or 0)
        ...
        if d["usuario_id"] <= 0 or d["contrato_id"] <= 0 or not d["competencia"] or not d["disciplina"]:
            raise ValueError("Preencha usuário, contrato, competência e disciplina.")
        if d["produzido_a1"] < 0 or d["horas_por_a1"] < 0 or d["horas_equipe"] < 0 or d["receita_faturada"] < 0:
            raise ValueError("Produção, horas e receita faturada não podem ser negativas.")
        return d

    def create(self, d):
        d = self._validate(d)
        new_id = self.repo.create(d)
        self.log(new_id, "create", "Realizado criado")

    def update(self, obj_id, d):
        d = self._validate(d)
        self.repo.update(obj_id, d)
        self.log(obj_id, "update", "Realizado atualizado")

    def delete(self, obj_id):
        self.log(obj_id, "delete", "Realizado excluído")
        self.repo.delete(obj_id)
...
class ProdutividadeCustoService(BaseProdService):
    ...
    def _validate(self, d):
        ...
        d["competencia"] = _normalize_competencia(d.get("competencia", ""))
        d["disciplina"] = str(d.get("disciplina", "")).strip()
        d["tipo_recurso"] = str(d.get("tipo_recurso", "")).strip()
        d["fornecedor_nome"] = str(d.get("fornecedor_nome", "")).strip()
        d["horas"] = to_float(d.get("horas", 0))
        d["custo_hora"] = to_float(d.get("custo_hora", 0))
        d["status_aprovacao"] = _normalize_status(d.get("status_aprovacao", "rascunho"))
        if d["custo_hora"] == 0 and d["tipo_recurso"].lower() in {"equipe", "interno", "interna"}:
            prm = self.parametro_repo.get_by_disciplina(d["disciplina"])
            if prm:
                d["custo_hora"] = float(prm["custo_hora_equipe"] or 0)
        d["custo_total"] = to_float(d.get("custo_total", 0)) or (d["horas"] * d["custo_hora"])
        ...
        if d["horas"] < 0 or d["custo_hora"] < 0 or d["custo_total"] < 0:
            raise ValueError("Horas e custos não podem ser negativos.")
        return d

    def create(self, d):
        d = self._validate(d)
        new_id = self.repo.create(d)
        self.log(new_id, "create", "Custo criado")

    def update(self, obj_id, d):
        d = self._validate(d)
        self.repo.update(obj_id, d)
        self.log(obj_id, "update", "Custo atualizado")

    def delete(self, obj_id):
        self.log(obj_id, "delete", "Custo excluído")
        self.repo.delete(obj_id)
