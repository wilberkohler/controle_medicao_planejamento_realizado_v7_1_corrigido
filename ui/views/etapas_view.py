...
class EtapasView(QWidget, FilterMixin):
    data_changed = Signal()
    ...
    def _build_ui(self):
        ...
        left_layout.addWidget(QLabel("Etapas"))
    ...
    def reload_data(self):
        ...
        for row in rows:
            ...
            self.table.setItem(r, 2, TextItem(row["codigo"]))
            self.table.setItem(r, 3, TextItem(row["descricao"]))
            self.table.setItem(r, 4, IntegerItem(row["ordem"]))
            self.table.setItem(r, 5, TextItem("Sim" if int(row["ativo"] or 0) == 1 else "Não"))
        self.lbl_1.setText(f"Ativas: {ativos}")
