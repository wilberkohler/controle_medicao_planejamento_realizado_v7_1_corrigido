...
class EntregaveisView(QWidget, FilterMixin):
    data_changed = Signal()
    ...
    def _build_ui(self):
        ...
        left_layout.addWidget(QLabel("Entregáveis"))
        ...
        self.table.setHorizontalHeaderLabels(["ID", "Contrato", "Etapa", "Grupo", "Código", "Descrição", "Unidade", "Ativo"])
        ...
        form.addRow("Unidade:", self.ed_unidade)
    ...
    def reload_data(self):
        ...
        for row in rows:
            ...
            self.table.setItem(r, 2, TextItem(row["etapa_codigo"]))
            self.table.setItem(r, 3, TextItem(row["grupo_codigo"]))
            self.table.setItem(r, 4, TextItem(row["codigo"]))
            self.table.setItem(r, 5, TextItem(row["descricao"]))
            self.table.setItem(r, 6, TextItem(row["unidade"] or ""))
            self.table.setItem(r, 7, TextItem("Sim" if int(row["ativo"] or 0) == 1 else "Não"))
    ...
    def on_select(self):
        ...
        self.ed_unidade.setText(obj["unidade"] or "")
