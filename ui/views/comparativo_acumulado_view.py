...
class ComparativoAcumuladoView(QWidget):
    ...
    def _build_ui(self):
        ...
        title = QLabel("Comparativo acumulado")
        ...
        self.table.setHorizontalHeaderLabels([
            "Contrato", "Competência", "Etapa", "Grupo", "Entregável",
            "Previsto Acum.", "Realizado Acum.", "Saldo Acum.",
            "% Previsto Acum.", "% Realizado Acum."
        ])
    ...
    def reload_data(self):
        ...
        for row in rows:
            ...
            self.table.setItem(r, 5, CurrencyItem(row["valor_previsto_acumulado"]))
            self.table.setItem(r, 6, CurrencyItem(row["valor_realizado_acumulado"]))
            saldo_item = CurrencyItem(saldo)
            if saldo > 0:
                saldo_item.setBackground(Qt.yellow)
            elif saldo < 0:
                saldo_item.setBackground(Qt.red)
            else:
                saldo_item.setBackground(Qt.green)
            self.table.setItem(r, 7, saldo_item)
            self.table.setItem(r, 8, FloatItem(row["percentual_previsto_acumulado"]))
            self.table.setItem(r, 9, FloatItem(row["percentual_realizado_acumulado"]))
        self.table.setSortingEnabled(True)
        self.lbl.setText(f"Registros: {len(rows)} | Saldo acumulado total: {br_number(saldo_total, 2)}")
