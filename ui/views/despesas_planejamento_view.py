...
class DespesasPlanejamentoView(QWidget, FilterMixin):
    data_changed = Signal()
    ...
    def _build_ui(self):
        ...
        ll.addWidget(QLabel("Despesas - Planejamento"))
        ...
        self.table = QTableWidget(0,8); self.table.setHorizontalHeaderLabels(["ID","Contrato","Competência","Categoria","Descrição","Fornecedor","Valor Prev.","Centro Custo"]); configure_table(self.table); self.table.itemSelectionChanged.connect(self.on_select); ll.addWidget(self.table)
        ...
        self.ed_comp = QLineEdit(); self.ed_desc = QLineEdit(); self.ed_forn = QLineEdit(); self.ed_valor = QLineEdit(); self.ed_cc = QLineEdit(); self.ed_obs = QTextEdit()
        for label, widget in [("Contrato:",self.cb_contrato),("Competência:",self.ed_comp),("Categoria:",self.cb_cat),("Descrição:",self.ed_desc),("Fornecedor:",self.ed_forn),("Valor Previsto:",self.ed_valor),("Centro de Custo:",self.ed_cc),("Observações:",self.ed_obs)]: form.addRow(label, widget)
        rl.addLayout(form)
        btns = QHBoxLayout()
    ...
    def reload_data(self):
        ...
        for row in rows:
            ...
            self.table.setItem(r,0,IntegerItem(row["id"])); self.table.setItem(r,1,TextItem(row["contrato_codigo"] or "(geral)")); self.table.setItem(r,2,TextItem(row["competencia"])); self.table.setItem(r,3,TextItem(row["categoria"])); self.table.setItem(r,4,TextItem(row["descricao"] or "")); self.table.setItem(r,5,TextItem(row["fornecedor"] or "")); self.table.setItem(r,6,CurrencyItem(row["valor_previsto"])); self.table.setItem(r,7,TextItem(row["centro_custo"] or ""))
        self.after_filter()
    ...
    def on_select(self):
        ...
        self.ed_comp.setText(obj["competencia"] or ""); self.ed_desc.setText(obj["descricao"] or ""); self.ed_forn.setText(obj["fornecedor"] or ""); self.ed_valor.setText(str(obj["valor_previsto"] or "")); self.ed_cc.setText(obj["centro_custo"] or ""); self.ed_obs.setPlainText(obj["observacoes"] or "")
    def get_data(self):
        return {"contrato_id": self.cb_contrato.currentData(), "competencia": self.ed_comp.text(), "categoria": self.cb_cat.currentText(), "descricao": self.ed_desc.text(), "fornecedor": self.ed_forn.text(), "valor_previsto": self.ed_valor.text(), "centro_custo": self.ed_cc.text(), "observacoes": self.ed_obs.toPlainText()}
    def novo(self): self.current_id = None; self.limpar()
    def limpar(self): self.table.clearSelection(); self.ed_comp.clear(); self.ed_desc.clear(); self.ed_forn.clear(); self.ed_valor.clear(); self.ed_cc.clear(); self.ed_obs.clear(); self.cb_cat.setCurrentIndex(0); self.cb_contrato.setCurrentIndex(0)
    def salvar(self):
        try:
